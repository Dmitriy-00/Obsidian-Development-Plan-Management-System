# Documentation/API/ComponentAPI.md
---
type: api-doc
category: component
created: <% tp.date.now() %>
...
```javascript
// Set state
component.state.set('data.value', 42);

// Subscribe to changes
component.state.subscribe('data.value', (newValue) => {
...
```javascript
// Listen for events
component.events.on('update', (data) => {
    console.log('Component updated:', data);
});

// Emit event
component.events.emit('update', { value: 42 });
```
